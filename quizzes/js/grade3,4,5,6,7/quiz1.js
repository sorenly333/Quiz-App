...
function showQuestion(index) {
    ...
    // Clone choices to avoid changing original order
    const shuffledChoices = shuffleArray([...q.choices]);

    let html = `<p><strong>Question ${index + 1}:</strong> ${q.question}</p>`;
    html += '<div class="choices">';
    shuffledChoices.forEach((choice, i) => {
        // Save the original index of the choice for hashing later
        const originalIndex = q.choices.indexOf(choice);
        html += `<label>
            <input type="radio" name="question${index}" value="${originalIndex}" 
            ${studentAnswers[index] == originalIndex ? 'checked' : ''}>
            ${choice}
        </label>`;
    });
    html += '</div>';
